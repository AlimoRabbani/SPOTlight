        ...
        if user is not None:
            if user.authenticate(form.password.data):
                if login_user(user, remember=form.remember.data):
                    current_app.logger.debug(url_for("user_views.devices_view", _external=True))
                    return redirect(request.args.get("next") or url_for("user_views.devices_view", _external=True))
            else:
...
def logout():
    logout_user()
    return redirect(url_for("common_views.index", _external=True))
