...
class Updater:
    ...
    def check_for_update(scheduler):
        try:
            request = urllib2.urlopen(Config.update_config["update_url"])
            response = request.read()
            version_info = json.loads(response)
            if float(version_info["version_id"]) > float(Config.update_config["version_id"]):
        ...
        scheduler.enter(int(Config.update_config["update_interval"]), 1, Updater.check_for_update, (scheduler, ))
    ...
    def download_file(updated_file):
        ...
        is_verified = Updater.verify_sign(Config.update_config["public_key"], updated_file["signature"], downloaded_file)
        if is_verified:
